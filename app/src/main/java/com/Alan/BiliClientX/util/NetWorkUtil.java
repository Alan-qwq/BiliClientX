...
public class NetWorkUtil {
     ...
     * 检查响应是否为DOCTYPE错误页面
     */
    private static boolean isDoctypeResponse(String str) {
        if (str == null) {
            return false;
        }
        return str.trim().toLowerCase(Locale.ROOT).startsWith("<!doctype");
    }

    /**
     * 检查Response是否为DOCTYPE错误页面（通过读取body前128字节）
     */
    private static boolean isDoctypeResponse(Response response) {
        ...
        try {
            ...
            int checkLen = Math.min(128, bodyBytes.length);
            String prefix = new String(bodyBytes, 0, checkLen, "UTF-8");
            return isDoctypeResponse(prefix);
    ...
    private static Response executeWithDoctypeRetry(OkHttpClient client, Request request) throws IOException {
        int maxRetries = Math.max(1, SharedPreferencesUtil.getInt(SharedPreferencesUtil.API_RETRY_MAX_TIMES, 5));
        long retryIntervalMs = Math.max(0L, (long) (SharedPreferencesUtil.getFloat(SharedPreferencesUtil.API_RETRY_INTERVAL_SECONDS, 0.1f) * 1000.0f));
    ...
    /**
     * 带DOCTYPE重试的获取body字符串
     */
    private static String getBodyStringWithDoctypeRetry(String url, ArrayList<String> headers) {
        int maxRetries = Math.max(1, SharedPreferencesUtil.getInt(SharedPreferencesUtil.API_RETRY_MAX_TIMES, 5));
        long retryIntervalMs = Math.max(0L, (long) (SharedPreferencesUtil.getFloat(SharedPreferencesUtil.API_RETRY_INTERVAL_SECONDS, 0.1f) * 1000.0f));
        ...
        for (int attempt = 1; attempt <= maxRetries; attempt++) {
            try (ResponseBody body = get(url, headers).body()) {
                ...
                if (!isDoctypeResponse(bodyString)) {
                    return bodyString;
                }
                if (attempt < maxRetries && retryIntervalMs > 0) {
                    try {
                ...
                lastResult = bodyString;
            } catch (Exception e) {
                // IOException from get() - will fall through to retry
            }
        }
        return lastResult;
    }
