        ...
        final List<SettingSection> sectionList = new ArrayList<>() {
            {
                ...
                add(new SettingSection("switch", "从历史位置播放", "player_from_last", getString(R.string.desc_fromlast),
                        "true"));
                add(new SettingSection("switch", "显示实时人数", "player_show_online", getString(R.string.desc_showonline),
                        "false"));
                add(new SettingSection("switch", "听视频模式", "player_audio_only", getString(R.string.desc_audio_only), "false"));
                add(new SettingSection("switch", "视频可缩放", "player_scale", getString(R.string.desc_scale), "true"));
                add(new SettingSection("switch", "缩放时可移动", "player_doublemove", getString(R.string.desc_doublemove),
                        "true"));

                add(new SettingSection("divider", "", "", "", ""));

                add(new SettingSection("choose", "显示方式", "player_display", getString(R.string.desc_display),
                        String.valueOf(Build.VERSION.SDK_INT < 26), new String[] { "TextureView", "SurfaceView" }));
                add(new SettingSection("choose", "解码方式", "player_codec", getString(R.string.desc_videocodec), "true",
                        new String[] { "硬件解码", "软件解码" }));
                add(new SettingSection("choose", "音频输出", "player_audio", getString(R.string.desc_audiocodec), "false",
